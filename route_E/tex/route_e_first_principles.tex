...
families an a-priori maximum.  A correction is essential: the canonical-contact
principle as stated does not by itself remove the one-dimensional abelian
\(g=1\) branch, since that algebra admits nondegenerate invariant forms up to
scale.  The rational curve, \(\cO(2)\) carrier, exactly three families, and
two-center divisor follow only after the explicit strengthening that the
contact be a nondegenerate Killing form (equivalently, that the family algebra
be semisimple).  Third, on that strengthened branch the contact direction equals
...
companion dynamics audits are mechanically executed, but their physics
promotion remains blocked by the open items listed below.
\end{abstract}
...
\item the rational family curve, the \(\cO(2)\) carrier, the two-center
  ...
  self-carried-index principle only after an explicit Killing-contact
  selection axiom is added; without it the rigorous result is the bound
  \(N_{\rm fam}\le3\) (Sec.~\ref{sec:families});
\item on that strengthened branch the contact direction is the Killing form
  of the family algebra, with existence, nondegeneracy, and uniqueness all
  consequences of simplicity (Sec.~\ref{sec:killing});
...
priori by three, with equality conditional on the explicit semisimple/Killing-
contact strengthening; and on that branch the Majorana contact direction is
the Killing form of that algebra.  These are
claimed as new assemblies of standard mathematics, not as new mathematics.

\section{The Principle Set and the Anchors}
\label{sec:principles}

All derivations below state their dependencies explicitly.  The four core
principles are inputs; the equality \(N_{\rm fam}=3\) additionally invokes
the Killing-contact selection axiom below.  The point of this note is to make
the input list minimal and explicit, and then to prove
...
\begin{assumption}[Killing-contact selection axiom]
\label{an:killingcontact}
For the purpose of selecting a family branch, the canonical contact is
required to be a nonzero multiple of the adjoint-trace form
\[
  B_{\rm ad}(x,y)=\Tr_{\aut(\Sigma)}
  \bigl(\operatorname{ad}_x\operatorname{ad}_y\bigr),
\]
and this form is required to be nondegenerate.  In the finite-dimensional
complex algebras on the genus ladder this is equivalent to semisimplicity.
This axiom strengthens Principle~\ref{pr:contact}; it is not implied by
invariance and nondegeneracy alone.
\end{assumption}
...
Note what is \emph{not} empirically anchored: the family number three.
It is selected in Sec.~\ref{sec:families} only conditionally on
Assumption~\ref{an:killingcontact}; without that axiom the principle set
proves only \(N_{\rm fam}\le3\).  No property of \(\zeta\) is anchored
beyond its existence as a coefficient.
...
\node[box] (pcont) at (6.0,3.2) {invariant-contact\\principle};
\node[box] (pplus) at (6.0,1.7) {Killing-contact\\selection axiom};
\node[thm] (t1) at (-5.0,0.0) {Thm.~\ref{thm:sixface}\\ \(\Spin(10)\) 16,
  \(\nu^c\) forced,\\ \(k_Y=5/3\)};
\node[thm] (t2) at (-0.5,0.0) {Cor.~\ref{cor:bound}: \(N\le3\)\\
  Thm.~\ref{thm:threefam} with H3\(^{+}\):\\
  \(g=0\), \(N=3\), \(\cO(2)\)};
...
\draw[arr] (pface) -- (t1);
\draw[arr] (pidx) -- (t2);
\draw[arr, dashed] (pcont) -- (pplus)
  node[midway, right] {\scriptsize strengthen};
\draw[arr] (pplus) -- (t2);
\draw[arr] (pplus) -- (t3);
\draw[arr] (t2) -- (t3);
\draw[arr] (t3) -- (t4);
...
\end{tikzpicture}
\caption{Logical dependency of the reconstruction.  The vertical dashed
edge marks the added Killing-contact strengthening; it is an assumption,
not a theorem.  Solid arrows are theorem dependencies.  The purple dashed
edge marks the point where a continuous datum enters, and
Theorem~\ref{thm:boundary} proves that no principle set of this kind can
supply it.}
...
enumeration; the genus ladder; the explicit one-dimensional abelian
counterexample to genus selection by invariant bilinearity; the
one-dimensionality of the invariant contact on the selected
\(\sltwo\) branch and the identity \(B=2\sqrt3\,K_{\rm tr}\); the Schur
complement; the \(\zeta\)-independence witness.
...
(i)~the four core principles; (ii)~the two empirical anchors;
(iii)~the Killing-contact/semisimplicity selection axiom;
(iv)~the
...
matter); (v)~the \(U(1)_R\) charge assignment of the hidden-messenger
sector --- the Majorana-only selection rule is \emph{derived} from it in
the companion note, but the assignment itself is an input; (vi)~the
...
\(\Spin^c\) Dolbeault index.  This section derives the upper bound from the
self-carried-index principle and derives those specific carrier data only
after adding the Killing-contact selection axiom.  The analytic inputs are
the standard Dolbeault--Hodge identification and Riemann--Roch with Serre
duality \cite{LawsonMichelsohn1989,GriffithsHarris1978}.
...
\label{lem:cartan}
Among \(g=0,1\), only \(g=0\) satisfies the \emph{strengthened}
Killing-contact condition that the contact be a nondegenerate multiple of the
Killing form, or equivalently that the family algebra be semisimple.  The
canonical-contact principle of Sec.~\ref{sec:principles} without this
strengthening does not select the genus.
\end{lemma}

\begin{proof}
For \(g=1\), \(\aut(\Sigma_1)\) is the one-dimensional abelian translation
algebra.  If \(e\) is a basis, \(B(e,e)=c\ne0\) defines a nondegenerate
symmetric ad-invariant form, so invariance and nondegeneracy alone do
\emph{not} exclude this branch; the arbitrary scale \(c\) is extra data.
Its Killing form nevertheless vanishes identically (\(\ad X=0\)), so it fails
the stated Killing-contact/semisimplicity strengthening.
...
\label{thm:threefam}
\mbox{}\par
Under the self-carried-index and canonical-contact principles, the existence
anchor, and the added Killing-contact/semisimplicity strengthening: the family
curve is \(\Sigma\simeq\CP^1\); the family space is the M\"obius algebra,
...
the strengthened Lemma~\ref{lem:cartan} leaves \(g=0\).  The rest is the
\(g=0\) row of the ladder.  Without the added strengthening, only
Corollary~\ref{cor:bound}, \(N_{\rm fam}\le3\), follows.
\end{proof}
...
the two fixed points of any regular semisimple M\"obius flow.
\end{proposition}
...
\(\Spin^c\) \(\cO(2)\) carrier as branch assumptions.  They become theorems
only after the Killing-contact/semisimplicity strengthening; under the original
principles the unconditional result is the upper bound \(N_{\rm fam}\le3\).
...
For \(g=0\), the dimension is \(h^0(K^{-m})=2m+1\).  Thus ``some
canonical bundle power'' would allow any odd family number.  The
self-carried clause is what
...
that it resembles the Killing form.  On the strengthened branch of
Theorem~\ref{thm:threefam} the resemblance becomes an identity, and
existence, nondegeneracy, and uniqueness follow from simplicity.
...
\begin{remark}[One strengthened condition, two consequences]
\label{rem:weld}
Conditional on Assumption~\ref{an:killingcontact}, the same simplicity that
selects \(g=0\) and hence \(N_{\rm fam}=3\)
(Lemma~\ref{lem:cartan}) also guarantees that the Killing contact exists,
is unique, and is invertible.  In the \(g=1\) branch an invariant,
nondegenerate contact \(B(e,e)=c\ne0\) can be written, but it is not the
adjoint-trace form and its normalization is extra data.  ``Exactly three
families'' and ``a nondegenerate Killing contact'' are thus two consequences
of the strengthened axiom, not of invariant bilinearity alone.
\end{remark}
...
\begin{datum}[Benchmark coefficient]
\label{dat:zeta}
\[
...
\[
  \begin{aligned}
  \lambda&=\sqrt{\zeta}
    =0.34502115743308964+0.10673473744038917\,i,\\
...
The six-face result and \(N_{\rm fam}\le3\) are theorems of the four core
principles.  The three-family/Killing branch additionally assumes the
\(\zeta\)-blind Killing-contact selection axiom, and the messenger theorem
uses the stated EFT inputs of Sec.~\ref{sec:messenger}.  Within that
explicitly conditional skeleton, exactly one continuous quantity remains:
the value of \(\zeta\).  This section proves that the remainder is not an
accident of insufficient cleverness.
...
\(\Spin(10)\)/three-family skeleton.  Every member satisfies all four core
principles, both anchors, and the Killing-contact selection axiom: the
...
item (iii) of Sec.~\ref{sec:sixface} --- and the remaining assumptions are
unchanged as \(\lambda\) varies.  Each member realizes
\(\zeta(M_\lambda)=\lambda^2\).
...
\boxed{
\begin{gathered}
\text{four core principles}\ \Longrightarrow\
 \bigl(\text{Thm.~\ref{thm:sixface} and }N_{\rm fam}\le3\bigr),\\[2pt]
\text{core + Killing-contact axiom + EFT inputs}\\
\Longrightarrow\ \text{the conditional skeleton}\\
\bigl(\text{Thms.~\ref{thm:threefam}, \ref{thm:killing},
\ref{thm:messenger}}\bigr),\\[2pt]
\text{every such }\zeta\text{-blind axiom set}\ \not\Longrightarrow\ \zeta ,\\
\text{hence a principles-only unconditional GUT proof is impossible.}
\end{gathered}
}
\]
...
\begin{observation}[Finite-class scan corroboration]
\label{obs:scans}
...
\begin{remark}[Diophantine diagnostic]
...
\path{code/audit8_dyn8_falsifiability_collection.py}, re-verifies
every number quoted in this section against its source ledger; its own
ledger is \path{output/audit8/dyn8_falsifiability_collection.json}.
...
\begin{description}
\item[Kinematic core (supersymmetry-agnostic; machine-verified).]\mbox{}\par
...
\item[Supersymmetric minimal completion (EXCLUDED as a slice family).]\mbox{}\par
...
is local to the excluded supersymmetric slice.

The required non-supersymmetric re-derivation was executed as a
four-stage audit series (ledgers under \texttt{output/audit9/}).
Vacuum stage: the \(210\) was built as an explicit rank-four tensor,
its full renormalizable potential constructed from a machine-tested
invariant basis (the five polynomial quartic contractions span only
four dimensions, and the Hodge--epsilon contraction is a genuine fifth
invariant vanishing identically on the singlet vev slice), and the
tree-level vacuum problem solved: the Pati--Salam vacuum is a
tree-level local minimum on \(\sim16\%\) of the sampled coupling
space, with every surviving scalar at or above the heavy gauge scale
--- so the proton-lifetime verdict is UNCHANGED by the real spectrum
--- while the \(210\)-only left--right vacuum is a tree-level minimum
only in \emph{rare} coupling regions concentrated at vev ratios near
\(0.6\)--\(0.7\) (eight of \(8800\) sampled points; consistent at
claim level with the classical minimization literature of the
\(210\), which an earlier fixed-ratio scan of ours had missed --- the
correction is recorded in the ledgers).  The fifth (parity-odd)
quartic invariant neither rescues nor kills any sampled point, so the
left--right chain is \(210\)-realizable at tree level in those
regions, with the adjoint (\(45\)-type) route as an alternative
rather than a necessity.  Source stage: with the corrected
GUT-normalized \(g_1\), imposing an additional top-like \(Y_\nu\) ansatz
gives the illustrative ratios \(y_t(M_X)/y_{\max}\simeq10\)
(Pati--Salam) and \(169\) (left--right); these are not an \(\SO(10)\)
lock theorem.  For the fixed archival kernel itself, reaching the
perturbative ceiling requires norm-suppression factors
\(\sigma_{\max}(Y_\nu^{\rm arch})/y_{\max}\simeq19\) and \(342\),
respectively.  The type-II order estimate falls short by
\(3.7\)--\(7.3\) orders.
The rescaled renormalizable benchmark also lies below an unflavored
Davidson--Ibarra floor.  These comparisons do not select a UV source:
they retain the archival Dirac-kernel shape and omit a global branch-local
flavor fit and flavored thermal kinetics.  The scale-decoupled
brane-instanton card therefore remains an unpromoted pricing hypothesis.
Contact stage: the machine audit verifies the uniform-rescaling covariance
\(Y_\nu\mapsto yY_\nu\), \(M_R\mapsto y^2M_R\).  With the physical
suppression scale kept positive, \(M_*' = |y|^2M_*\) and
\(\zeta'=e^{2i\arg y}\zeta\), while the contact fraction and projective
contact direction are unchanged.  Thus the numerical value of \(\zeta\)
is exactly invariant only for a uniform positive-real rescaling \(y>0\);
for a complex rescaling its phase covaries.
Consistently with Theorem~\ref{thm:boundary}, the contact coefficient
is an input in every branch; only its bookkeeping moves.

\item[String-conditional pricing cards (unpromoted).]
Three cards price string-motivated escapes without deriving them.  The
brane-instanton Majorana card buys the tower \emph{scale} but provides no
texture compression: its effective Takagi data carry the full
twelve-parameter Majorana content.  The protection card proves an exact
additive-Abelian no-go: allowing \(NLH\) and \(XN\) gives
\(q(XLH)=q(XX)\).  Its historical numerical selectivity gap is blocked
because it imported the invalid messenger ceiling of K7.  The
supersymmetry-breaking bridge is only a toy one-loop scan: its nominal
left--right lower edge permits \(M_I\simeq101\,\mathrm{GeV}\); imposing the
illustrative requirement \(M_I\ge10\,\mathrm{TeV}\) moves the grid edge to
\(\log_{10}(M_{\rm SS}/{\rm GeV})=11.85\).  Experimental intermediate-scale
bounds, threshold envelopes, an independent root solve, and proton-decay
uncertainties remain open.  These cards are conditional interpretations
under the promotion discipline of the companion note's string appendix;
they must not be cited as established evidence.
\end{description}
...
Corollary~\ref{cor:bound}, hence the self-carried-index principle
...
falsifier.  (Ledger: \path{route_E/output/route_e_first_principles.json}.)
...
Theorem~\ref{thm:killing}.  Within the declared nuisance-prior card,
every sampled contact fraction exceeds (0.01) (sample fraction (1.000));
this is a conditional regression statistic, not a likelihood-weighted data
posterior.  The refit audit further finds that, over a
thousand kernel-perturbed refits (perturbations up to \(30\%\)) the
contact fraction never falls below \(0.01\).  The same audit
establishes that the benchmark card's \(\theta_{23}\) tension is a
frozen-anchor property: the Majorana sector refits the current
oscillation centrals \emph{identically} at unchanged Dirac kernels,
and a \(5\%\) charged-lepton kernel perturbation alone absorbs the
shift.
(Ledgers: \path{output/audit1/dyn4b_unconditional_zeta.json},
\path{dyn4c_kernel_dirac_refit.json}.)
...
scan.  (Ledgers: \path{output/audit2/dyn3_proton_d5_kill_criterion.json},
\path{output/audit3/dyn2b_rescue_scan.json}.)
...
falsifiability statement this framework currently owns.  Retested with
the \emph{computed} \(210\) spectrum and then with the computed
\(\overline{126}\) spectrum at tree level (the extended-survival
assumption replaced by real Hessians): the lifetime is unchanged at
all sampled percentiles, so the entry now rests on three independent
spectrum treatments.
(Ledgers: \path{output/audit9/dyn9_nonsusy_intermediate.json},
\path{dyn9b1b_210_quartic_descent.json},
\path{dyn9b1c_eps_and_126_quartics.json}.)
...
branch.  Caveat from the vacuum stage (corrected by the ratio scan):
the left-right chain's \(210\)-only vacuum is a tree-level minimum
only in rare coupling regions (vev ratios near \(0.6\)--\(0.7\)),
consistent with the classical \(210\) minimization literature; the
adjoint breaking route remains an equally available alternative.
(Same ledgers as K4, plus
\path{output/audit9/dyn9b1d_lr_ratio_scan.json}.)

\item[K6.] \textbf{Thermal leptogenesis diagnostic} [not promoted].
The historical unflavored scans are arithmetic regressions, not posterior
probabilities: \(M_1=2.38\times10^{10}\,\mathrm{GeV}\) lies in the
tau-resolved regime.  On the non-supersymmetric branch the corrected
Standard-Model Davidson--Ibarra ceiling is
\(2.3078\times10^{-6}\), and the unflavored asymmetry is about six times
smaller than in the supersymmetric regression.  Only the
gravitino-specific ceiling is absent; reheating history and initial abundance
remain unspecified.  Publication requires at least two-flavor kinetics with
spectator effects and branch-local thermal inputs.
(Ledgers: \path{output/audit7/dyn7_leptogenesis_argzeta.json},
\path{output/audit7/dyn7_flavor_regime_gate.json},
\path{output/audit9/dyn9b3_nonsusy_leptogenesis.json}.)

\item[K7.] \textbf{Messenger canonical-normalization diagnostic}
[invalid pending rederivation].  The seesaw Schur identity is exact, but the
displayed \(XN\) interaction is bilinear rather than a cubic Yukawa coupling,
so the asserted one-loop anomalous dimension and spurion ceilings are not
defined by the displayed action.  Tree matching instead gives
\(\delta Z=|\zeta|/3=0.0434773\), and the displayed charges allow
\(XLH_u\).  No numerical messenger ceiling is promoted.
(Ledgers: \path{output/audit5/dyn5_messenger_one_loop.json},
\path{output/audit5/dyn5_model_validity.json}.)

\item[K8.] \textbf{Heavy-neutrino/gauge-scale coexistence}
[string-conditional fixed-tower diagnostic].  In the replayed archival
tower, the two heavier right-handed neutrinos sit \emph{above} the
intermediate gauge scale (factors \(39\) and \(4.8\times10^{3}\) on the
Pati--Salam chain), beyond a perturbative \(f v_R\) source.  The instanton
benchmark permits this ordering but does not require it for a generic
independently fitted tower.  The source-comparison audit
(the ansatz-dependent \(10/169\) top-like diagnostic, the actual
\(19/342\) archival-kernel ceiling suppression, the type-II estimate,
and the unflavored leptogenesis floor) only compares two priced benchmarks.
Since the global
non-supersymmetric flavor fit and flavored kinetics remain open, it does not
select a source and the fixed-tower coexistence statement is not a promoted
branch requirement;
the underlying string card also remains unpromoted.  Measuring an
intermediate-scale gauge boson together with heavy-neutrino masses
below it falsifies the scenario.
(Ledgers: \path{route_d/output/d3_instanton_majorana_pricing.json},
\path{output/audit9/dyn9b2_nonsusy_flavor_refit.json}.)

\item[K9.] \textbf{Zero-mode selectivity} [string-conditional].
The additive-Abelian no-go is exact: once \(NLH\) and \(XN\) are allowed,
\(q(XLH)=q(XX)\).  The replayed historical-invalid values
\(\Delta S=4.66\) (refreshed) and \(3.18\) (loose) inherit the invalid
DYN-5 messenger ceiling and are therefore
blocked pending an interacting messenger rederivation; they are not exclusion
bounds on a concrete embedding.
(Ledger: \path{route_d/output/d4_stueckelberg_protection.json}.)

\item[K10.] \textbf{Supersymmetry-breaking bridge} [unpromoted toy scan].
The one-loop minimal-content scan returns a left-right window
\(\log_{10}M_{\rm SS}\in[10.30,15.55]\), but its lower edge has only
\(M_I\simeq101\,\mathrm{GeV}\).  No branch-specific experimental lower bound,
threshold nuisance envelope, complete proton uncertainty, or independent
global root audit is included.  The Pati--Salam toy slice has no point, but no
model-class exclusion or ultraviolet bridge is promoted.
(Ledger: \path{route_d/output/d5_susy_breaking_bridge.json}.)
...
stands.  (Ledger: \path{output/audit1/dyn4b_unconditional_zeta.json}.)
\end{enumerate}
...
is an input.  Under a uniform scalar rescaling
\(Y_\nu\mapsto yY_\nu\), seesaw covariance gives
\(M_R\mapsto y^2M_R\).  With \(M_*'>0\), one has
\(M_*'=|y|^2M_*\), \(\zeta'=e^{2i\arg y}\zeta\), and an unchanged
contact fraction; the contact direction is unchanged projectively.
Consequently \(\zeta\) itself is exactly invariant only for \(y>0\)
(machine-verified to \(10^{-16}\)); a complex rescaling rotates its phase.
This is scaling covariance, not an \(\SO(10)\) lock theorem (ledger
\path{output/audit9/dyn9b2_nonsusy_flavor_refit.json}).  Likewise
...
Sec.~\ref{sec:sixface}.
\item[Conditional theorem after an added axiom.]
The \(g=0\) selection, \(C=\CP^1\), \(T\simeq\cO(2)\),
\(N_{\rm fam}=3\), two-center divisor, and Killing-contact identification
require the explicit Killing-contact/semisimplicity strengthening.  A
one-dimensional abelian algebra has nondegenerate invariant forms, so the
original contact principle alone does not eliminate \(g=1\).
...
the stated kind.
\item[Mechanically executed dynamics evidence (July 2026).]
The dependency graph and three pricing cards are reproducible at the ledger
level.  The fail-closed collector passes its disclosure checks with
\path{physics_promotion_allowed=false}; this is not dynamics closure.
The finite supersymmetric benchmark slice has a conditional exclusion, while
the non-supersymmetric branch remains a preliminary model candidate.
\item[Still open.]
The non-supersymmetric scalar potential and spectrum; a global branch-local
Spin(10) flavor fit; physical-basis proton-decay rotations; an interacting and
selection-safe messenger action; recomputed string pricing; and tau-resolved
Boltzmann/density-matrix kinetics with spectator, reheating, and initial-state
inputs.
...
\begin{itemize}
\item Paper source: \path{route_E/tex/route_e_first_principles.tex}.
\item First-principles audit (plain \texttt{python3} + \texttt{numpy},
  exact rational arithmetic for the enumeration):
  \texttt{python3} \path{route_E/code/verify_route_e_first_principles.py};
  ledger \path{route_E/output/route_e_first_principles.json} and
...
\item Dependency-closure audit:
  \texttt{python3} \path{route_E/code/verify_route_e_dependency_closure.py};
  ledger \path{route_E/output/route_e_dependency_closure.json} and
...
\item Dependency roadmap: \path{route_E/ROADMAP.md} (removed,
...
\item Dynamics-audit ledgers (Sec.~\ref{sec:falsifiability}): scripts
  \path{code/audit0...audit9_dyn*.py} with ledger pairs under
  \path{output/audit0..9/}; string-conditional cards
  \path{route_d/code/verify_d3..d5_*.py} with ledgers under
  \path{route_d/output/}; collector
  \path{code/audit8_dyn8_falsifiability_collection.py} with ledger
  \path{output/audit8/dyn8_falsifiability_collection.json}, which
...
\item Negative-boundary flags recorded in the JSON ledgers:
  \path{zeta_value_derived}\texttt{: false};
  \path{no_extra_chiral_sector_assumption_discharged}\texttt{: false};
  \path{route_b_r_selection_rule_derived}\texttt{: false};
  \path{companion_phenomenology_audits_replayed}\texttt{: false};
  \path{witten_global_anomaly_cited_not_computed}\texttt{: true};
  \path{rank_ge_17_min_dims_cited_not_computed}\texttt{: true};
  \path{pslt_only_unconditional_gut_proof_claimed}\texttt{: false}.
\end{itemize}

\section{Conclusion}

This note compresses the companion note's branch assumptions into four
explicit principles plus one clearly separated Killing-contact selection
axiom and pushes the theorem boundary to its provable maximum.  The
\(\Spin(10)\) half-spinor with its forced sixteenth state and the upper bound
\(N_{\rm fam}\le3\) follow from the core principles.  The rational family
curve with its Poincar\'e--Hopf two-center divisor, exactly three
automorphism-protected families, and the Killing-form contact follow
conditionally on the added selection axiom.  The hidden-messenger mechanism
is form-unique on that branch, while the single complex coefficient
\(\zeta\) provably is not fixed.  Every external
...
precisely recorded in the status ledger: conditional data plus mechanically
executed but unpromoted dynamics lanes requiring a global non-supersymmetric
flavor fit, valid messenger action, recomputed string pricing, and flavored
thermal kinetics.  Theorem~\ref{thm:boundary} explains why the continuous
coefficient cannot be talked away; it does not discharge those physical
blockers.
