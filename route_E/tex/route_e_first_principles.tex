...
rather than a necessity.  Source stage: the perturbative ceiling caps the
neutrino Dirac scale a factor \(9\) (Pati--Salam) to \(159\)
(left--right) below its \(\mathrm{SO}(10)\)-locked value, the type-II
escape falls short by \(3.7\)--\(7.3\) orders, and thermal
leptogenesis drops below the Davidson--Ibarra floor under the rescaled
renormalizable source --- leaving the scale-decoupled brane-instanton
source (the first pricing card below) as the only quantified surviving
scenario.  Contact stage: a machine-verified invariance theorem shows
the normalized contact content (coefficient, direction, and contact
fraction) is \emph{exactly} invariant under a rescaling of the Dirac
kernel; only the suppression scale \(M_*\) is branch-local.
...
and the refit audit lifts the kernel conditionality --- over a
...
shift.
(Ledgers: \texttt{output/audit1/dyn4b\_unconditional\_zeta.json},
\texttt{dyn4c\_kernel\_dirac\_refit.json}.)
...
spectrum treatments.
(Ledgers: \texttt{output/audit9/dyn9\_nonsusy\_intermediate.json},
\texttt{dyn9b1b\_210\_quartic\_descent.json},
\texttt{dyn9b1c\_eps\_and\_126\_quartics.json}.)
...
\texttt{output/audit9/dyn9b1d\_lr\_ratio\_scan.json}.)

\item[K6.] \textbf{Thermal leptogenesis} [branch-tagged; soft
criterion].  On the excluded supersymmetric slice, thermal unflavored
leptogenesis fails by \(\sim1.5\) orders with the wrong central sign
(success probability \(0.4\%\); a \(\times3\)--\(10\) enhancement
lifts it to \(34\)--\(45\%\)).  On the surviving non-supersymmetric
branch the rerun with the Standard-Model loop function and Higgs
normalization is a factor \(\sim6\) \emph{harder} (posterior success
\(0/4000\) unflavored; the enhancement sweet spot moves to
\(\times30\)--\(100\)), but the gravitino/reheating ceiling dissolves
by the branch itself: thermal production is unconstrained and
non-thermal production opens as a qualitatively new escape, alongside
flavored effects and a near-degenerate instanton-sourced tower.  A
soft constraint on the source scenario, not a kill.
(Ledgers: \texttt{output/audit7/dyn7\_leptogenesis\_argzeta.json},
\texttt{output/audit9/dyn9b3\_nonsusy\_leptogenesis.json}.)
...
[string-conditional; UPGRADED to a branch requirement].  The instanton
Majorana source \emph{requires} the two heavier right-handed neutrinos
to sit \emph{above} the intermediate gauge scale (factors \(39\) and
\(4.8\times10^{3}\) on the Pati--Salam chain) --- impossible for a
renormalizable perturbative source.  The source-selection audit
(lock tension, type-II failure, leptogenesis floor) leaves this
scale-decoupled source as the \emph{only} quantified scenario for the
alive branch with viable leptogenesis, so the coexistence prediction
upgrades from an optional card to a requirement of that branch; the
underlying string card itself remains unpromoted.  Measuring an
intermediate-scale gauge boson together with heavy-neutrino masses
below it falsifies the scenario.
(Ledgers: \texttt{route\_d/output/d3\_instanton\_majorana\_pricing.json},
\texttt{output/audit9/dyn9b2\_nonsusy\_flavor\_refit.json}.)
...
is an input, and its branch-locality is now a \emph{theorem}: under a
rescaling of the Dirac kernel the normalized contact content
(coefficient, direction, contact fraction) is exactly invariant, and
only the suppression scale \(M_*\) moves (machine-verified to
\(10^{-16}\); ledger
\texttt{output/audit9/dyn9b2\_nonsusy\_flavor\_refit.json}).  Likewise
