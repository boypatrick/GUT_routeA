#!/usr/bin/env python3
"""Tightened replay of the signed-interference d=5 proton proxy.

No web lookup is used.  The previous signed-interference audit scanned
triplet, neutralino, and higgsino phases, but it used the historical
S_T=1e-5 display point and the old single hadronic prefactor.  This replay
uses the already-computed worst-phase channel rows and applies

    margin_new = margin_old * (S_T_old/S_T_new)^2 * (K_old/K_channel)

with S_T_new=7.5e-6 and channel-dependent chiral/lattice prefactors.  Because
the input channel rows are already worst over the scanned CP phases, a passing
result here means the branch does not rely on accidental destructive phase
interference within that grid.
"""
...
SIGNED_CHANNELS = ROOT / "output" / "signed_interference_d5" / "channel_scan.csv"
CHIRAL = ROOT / "output" / "chiral_lattice_d5" / "summary.json"
TIGHTENED = ROOT / "output" / "tightened_triplet_filter" / "summary.json"
SOFT_REPLAY = ROOT / "output" / "soft_spectrum_d5_replay" / "summary.json"
OUT = ROOT / "output" / "interference_tightened_d5_replay"
...
def read_channel_rows() -> list[dict[str, Any]]:
    ...
    with SIGNED_CHANNELS.open(newline="", encoding="utf-8") as handle:
...
def replay() -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    signed = read_json(SIGNED_SUMMARY)
    chiral = read_json(CHIRAL)
    tightened = read_json(TIGHTENED)
    soft = read_json(SOFT_REPLAY)
    widths = chiral_widths(chiral)
    display_st = float(signed["display_triplet_filter"])
    ...
    for row in read_channel_rows():
        ...
        for case in ["central", "max_width", "min_width"]:
            ...
            future_margin = margin * target_current / 1.0e35
            best_phase_margin = margin * row["phase_spread"] ** 2
            channel_rows.append(
                {
                    **row,
                    "channel_class": cls,
                    ...
                    "margin_replayed_current_worst_phase": margin,
                    "margin_replayed_current_best_phase_proxy": best_phase_margin,
                    "future_margin_1e35_worst_phase": future_margin,
                    "passes_replayed_current_worst_phase": margin >= 1.0,
                    "passes_future_1e35_worst_phase": future_margin >= 1.0,
                    "current_target_years": target_current,
                }
            )

    point_rows = point_summaries(channel_rows)
    summary = summarize(point_rows, channel_rows, signed, soft, st_target)
...
def point_summaries(channel_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for key, rows in sorted(grouped.items()):
        worst_current = min(rows, key=lambda row: row["margin_replayed_current_worst_phase"])
        worst_future = min(rows, key=lambda row: row["future_margin_1e35_worst_phase"])
        max_spread = max(row["phase_spread"] for row in rows)
        min_spread_at_worst = worst_current["phase_spread"]
        point_rows.append(
            {
                "filter_label": key[0],
                "scenario": key[1],
                ...
                "worst_triplet_phase": worst_current["triplet_phase"],
                "worst_current_margin": worst_current["margin_replayed_current_worst_phase"],
                "worst_current_ST_max": worst_current["S_T_max_replayed"],
                "worst_current_phase_spread": min_spread_at_worst,
                "worst_future_channel": worst_future["channel"],
                "worst_future_pair": worst_future["pair"],
                "worst_future_margin_1e35": worst_future["future_margin_1e35_worst_phase"],
                "max_phase_spread": max_spread,
                "all_channels_pass_current": all(row["passes_replayed_current_worst_phase"] for row in rows),
                "all_channels_pass_future_1e35": all(row["passes_future_1e35_worst_phase"] for row in rows),
            }
        )
    return point_rows
...
def summarize(
    ...
    for filt in sorted({row["filter_label"] for row in point_rows}):
        ...
        for case in ["central", "max_width", "min_width"]:
            ...
            safe = [row for row in rows if row["all_channels_pass_current"]]
            unsafe = [row for row in rows if not row["all_channels_pass_current"]]
            ...
            aligned_safe = [row for row in aligned if row["all_channels_pass_current"]]
            most_marginal = min(safe, key=lambda row: row["worst_current_margin"]) if safe else None
            nearest_unsafe = max(unsafe, key=lambda row: row["worst_current_margin"]) if unsafe else None
            worst_future = min(rows, key=lambda row: row["worst_future_margin_1e35"])
            by_case[case] = {
                "positive_points": len(rows),
                "current_safe_points": len(safe),
                "current_unsafe_points": len(unsafe),
                "future_1e35_safe_points": len(future_safe),
                "aligned_positive_points": len(aligned),
                "aligned_current_safe_points": len(aligned_safe),
                "current_safe_fraction": len(safe) / len(rows) if rows else 0.0,
                "aligned_current_safe_fraction": len(aligned_safe) / len(aligned) if aligned else 0.0,
                ...
                "max_phase_spread": max(row["max_phase_spread"] for row in rows),
                "most_marginal_current_safe": trim_point(most_marginal),
                "nearest_current_unsafe": trim_point(nearest_unsafe),
                "worst_future_1e35": trim_point(worst_future),
            }
        by_filter[filt] = by_case

    preferred = by_filter["omegaR_0.1_kappa_100"]["max_width"]
    allblock = by_filter["all_blocks_kappa_100"]["max_width"]
    verdict = {
        "S_T_target": st_target,
        "preferred_filter": "omegaR_0.1_kappa_100",
        "preferred_max_width_safe_points": preferred["current_safe_points"],
        "preferred_max_width_positive_points": preferred["positive_points"],
        "preferred_max_width_worst_current_margin": preferred["global_worst_current_margin"],
        "preferred_max_width_worst_future_1e35_margin": preferred["global_worst_future_1e35_margin"],
        "preferred_future_amplitude_suppression_needed": preferred[
            "additional_amplitude_suppression_for_future"
        ],
        "preferred_max_phase_spread": preferred["max_phase_spread"],
        "allblock_max_width_safe_points": allblock["current_safe_points"],
        "allblock_max_width_positive_points": allblock["positive_points"],
        "allblock_max_width_worst_current_margin": allblock["global_worst_current_margin"],
        "soft_replay_preferred_max_width_margin": soft["verdict"][
            "preferred_max_width_worst_current_margin"
        ],
        "signed_reference_old_worst_margin": signed["by_filter_label"]["omegaR_0.1_kappa_100"][
            "most_marginal_safe"
        ]["worst_margin"],
        "interpretation": (
            "Worst-phase signed replay at S_T=7.5e-6 and the conservative "
            "chiral/lattice max-width envelope keeps the preferred "
            "omegaR=0.1,kappa=100 branch safe on every audited positive soft "
            "point.  Since the replay uses max-phase amplitudes, this pass does "
            "not rely on accidental destructive interference.  The all-block "
            "fallback remains unsafe, and a uniform 1e35 yr stress still needs "
            "additional O(1) suppression."
        ),
    }
    return {
        "note": "No web lookup used. Tightened signed-interference d=5 replay with chiral/lattice envelope.",
        "input_files": {
            "signed_interference_d5": str(SIGNED_SUMMARY),
            "signed_channel_scan": str(SIGNED_CHANNELS),
            "chiral_lattice_d5": str(CHIRAL),
            "tightened_triplet_filter": str(TIGHTENED),
            "soft_spectrum_d5_replay": str(SOFT_REPLAY),
        },
        "formula": "margin_new = margin_old * (S_T_old/S_T_new)^2 * (K_old/K_channel)",
        "phase_statement": "channel rows are already worst over the scanned neutralino/higgsino phases",
...
def write_outputs(channel_rows: list[dict[str, Any]], point_rows: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    channel_fields = [
        ...
        "passes_replayed_current_worst_phase",
        "passes_future_1e35_worst_phase",
    ]
    point_fields = [
        "filter_label",
        "scenario",
...
def write_report(summary: dict[str, Any]) -> None:
    lines = [
        "# Tightened signed-interference d=5 replay",
        "",
        ...
        "```",
        "",
        "The input channel rows are already worst over the scanned neutralino",
        "and higgsino phases, so this is a worst-phase pass/fail replay.",
        "",
        "## Max-width envelope",
        "",
        "| filter | current safe/positive | worst current margin | future safe/positive | worst future margin | max phase spread |",
        "|---|---:|---:|---:|---:|---:|",
    ]
...
def main() -> None:
    ...
    print("Tightened signed-interference d=5 replay")
