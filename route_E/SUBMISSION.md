...
| claim | verdict | notes |
|---|---|---|
| Family symmetry GENERATED as the automorphism algebra of its own carrier (not imposed) | **ADJACENT** | Octonion/sedenion/triality routes to "why three" exist (S3 from Aut(S), Spin(8) triality, exceptional Jordan algebra) but use a different mechanism (discrete outer automorphisms of a division-algebra structure, not the automorphism Lie algebra of the family carrier).  SU(2)-flavor models exist but the symmetry is imposed with flavons.  SUB-1 must cite and distinguish in one paragraph. |
| Self-carried bound N_fam <= 3 | **ADJACENT** | The paradigm "generation number = compactification index" is classical (CHSW, heterotic N_gen = index/c_3 -- already cited).  The specific self-carried genus ladder was not found.  This is the unconditional result. |
| Conditional equality N_fam = 3 from the explicit H3+ Killing-contact axiom | **CORRECTED / ADJACENT** | Original H3 was insufficient: on the one-dimensional abelian branch, B(e,e)=c != 0 is invariant and nondegenerate although the Killing form vanishes.  Equality is claimable only after adding the nondegenerate adjoint-trace/Killing-contact (semisimplicity) axiom; its physical motivation or UV realization remains open. |
| Majorana contact direction = Killing form of the selected family algebra | **CLEAR / CONDITIONAL** | No hit for Killing-form Majorana textures; texture literature (zeros, cyclic, etc.) is unrelated.  The identity is exact on the H3+-selected g=0 branch and must not be advertised as a consequence of original H3 alone. |
