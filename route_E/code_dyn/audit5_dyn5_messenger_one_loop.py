#!/usr/bin/env python3
"""DYN-5 historical one-loop regression (scientific claim invalid).

This file preserves the original numerical pipeline for provenance only.  It
incorrectly treats the quadratic mass mixing ``lambda X N`` as a cubic Yukawa
interaction.  Its putative loop coefficient is therefore not generated by
the displayed action, and its R enumeration omits the allowed ``X L H_u``
operator.  The fail-closed replacement is
``audit5_dyn5_model_validity.py``; no one-loop physics result from this file
is publishable.
...
payload = {
    ...
    "superseded_by": "route_E/code_dyn/audit5_dyn5_model_validity.py",
    "invalid_reasons": [
        "lambda X N is a quadratic mass mixing, not a cubic Yukawa tensor",
        "the displayed R charges allow X L H_u",
        "tree-level Kahler matching precedes any interacting-loop calculation",
    ],
}
(OUT / "dyn5_messenger_one_loop.json").write_text(
    json.dumps(payload, indent=2) + "\n")

md = ["# DYN-5: historical one-loop regression (invalid claim)", "",
...
print(f"\nDYN-5: {n_pass}/{len(CHECKS)} historical arithmetic checks; "
      "scientific status = invalid_pending_rederivation; use "
      "audit5_dyn5_model_validity.py.")
if not all_pass:
    raise SystemExit(1)
