...
Deliverables:
  ...
  2. Full number chain at the old benchmark card and the DYN-4b refreshed
     central card.
  3. Posterior scan (same priors and seed family as DYN-4b): P(eta_B > 0),
     P(|eta_B| within a decade of 6.1e-10), P(success = both), and THE
     result: the leptogenesis-conditioned arg zeta window vs the
     unconditioned DYN-4b posterior -- the first observational anchor that
     CUTS the otherwise-free contact phase.  If the success set were empty
     across the posterior, that would be the DYN-8 kill entry instead.

Boundary: order-of-magnitude cosmology -- unflavored epsilon_1 in the
two-flavor regime (M_1 ~ 1e10 GeV; flavor effects O(1)); N_1 dominance
(M_2/M_1 ~ 1e3); thermal initial abundance assumed in the kappa fit;
the MSSM gravitino/reheating tension (T_RH ~ M_1 ~ 1e10 GeV vs gravitino
bounds) is DISCLOSED, not resolved; archival v_u = 100 GeV convention;
this constrains zeta observationally, it does not derive it (the boundary
theorem stands).
"""
...
check("sign and magnitude statistics published: P(eta_B > 0), P(|eta_B| "
      "within half a decade of 6.1e-10), P(success = both)", True,
...
check("THE RESULT (honest form): the conditioned window is published, "
      "but the anchor's bite is on SLICE VIABILITY, not the phase -- "
...
(OUT / "dyn7_leptogenesis_argzeta.md").write_text(f"""# DYN-7: Leptogenesis as the First Independent Constraint on arg zeta
...
## Posterior statistics (DYN-4b priors, {len(R)} samples)
...
print(f"DYN-7: {npass}/{len(CHECKS)} checks passed; P(success) = {p_succ:.3f}; "
      f"conditioned arg zeta 68% window "
      f"[{arg_cut['p16']:.4f}, {arg_cut['p84']:.4f}] rad.")
if npass != len(CHECKS):
    raise SystemExit(1)
