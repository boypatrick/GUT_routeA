...
results, but no complete pre-portal lane.

- **Exact gauge bordism, incomplete regulator:**
  `Omega_5^Spin(BSp(2))=Z2`, generated by a unit `Sp(2)` instanton on `S4`
  times the nonbounding spin circle.  The representation characters are
  `4:-1`, `5:+1`, `10:+1`; the two Dirac `5`s are gauge-bordism trivial.
  The revised card passes **39/39**.  The two target-space mapping-torus signs
  are nevertheless independent and unselected.  PV moments and local theta
  periodicity do not supply the missing full regulator or heavy/gravitational
  APS determinant ratio.
- **Canonical coupled `B=1` BVP (20/20):** the nonlinear hedgehog-sector `F,s` solve reaches
  `E/(4 pi)=6.1282155`, `s(0)=0.9171289646`, `B=1` at `R=16`.  Its canonical
  checksum is
  `81104f59a5f3f4337739fc2a217cafc8da91581c9f1fb40b4fdba6ce0f948d59`.
  The sparse constrained/projected `n+s` Hessian is differentiated exactly;
  the separate diquark/gauge/ghost blocks are not one same-grid aggregate.
  The sampled field has gradient density about `.35-.43` and its lattice
  estimator only reaches `B_a=.625` on the finest displayed grid.  Negative
  curvatures are off-shell diagnostics; aggregate-Hessian, discrete
  stationarity, topology, continuum stability, fermions, HMC, and quantum
  closure remain false.
- **Same checksum, physical-moduli no-go (25/25):** the hedgehog orbit is
  `SU(2)/Z2=SO(3)`, while the scalar `CP1` mode has volume-divergent norm.
  The static Callias boundary index is zero for the constant asymptotic mass.
  Moreover, a gapped two-level mass in a trivial rank-two bundle obeys
  `c1^2=0` and cannot realize `c1=x+2y`, whose square is `4xy`.  The correctly
  typed WZW character is only a restricted local ansatz; no physical `O(2)`
  determinant, CPT regulator, or full gauge-basic descent is obtained.
- **Portal (15/15):** the new independent conjunction still gives all three lane
  closures false.  No portal source has been written.

Ordered next work is: (1) complete a common APS/PV/bosonic regulator and
heavy threshold; (2) discretely re-relax the same field before interpreting
its Hessian and then add dynamical 4d fermions; (3) reformulate the family
question on the actual `SO(3)`/FR cover or build a rank-three mass family that
passes the new obstruction.  The degree-one Route-E portal remains fourth.

Full formulas and reproducible anchors are in
`route_f/tex/another_physics_route_e_derivation_ledger.tex` and the three
standalone `route_f/tex/ap_e6_*` notes.
...
## 2026-06-07 Taipei Route-C P8 comparative theory scorecard

status:
  Implemented P8 for the Route-C bootstrap workspace.  The new script compares
  `SU(5)`, Pati-Salam, `Spin(10)`, `E6`, and superstring-derived completions
  using the explicit P1-P7 gates.  It is a hard-gate scorecard, not a weighted
  score and not a completed GUT proof.

files changed:
  - `route_c_bootstrap/code/theory_comparison_scorecard.py`
  - `route_c_bootstrap/output/theory_comparison_scorecard.json`
  - `route_c_bootstrap/output/theory_comparison_scorecard.md`
  - `route_c_bootstrap/tex/route_c_derivation_ledger.tex`
  - `route_c_bootstrap/README.md`
  - `route_c_bootstrap/output/README.md`
  - `route_c_bootstrap/ROUTE_C_WORKPLAN.md`
...
content:
  ...
  - Recorded conservative verdicts:
      `SU(5)` is a conditional branch if the single-object filter is relaxed,
      Pati-Salam is a conditional intermediate face but not a final simple
      object,
      `Spin(10)` is the leading conditional finite field-theory branch,
      `E6` is conditional with exotics-lifting debt,
      superstring-derived completions are a UV-completion comparison class not
      forced by P1-P7.

verification:
  - Ran `python3 route_c_bootstrap/code/theory_comparison_scorecard.py`.
  - Generated:
...
boundary:
  P8 closes the first Route-C pass as a comparative filter.  It does not
  convert `Spin(10)` into a completed action-level GUT and does not claim that
  superstring theory is required.

next:
  - Stop broadening the comparison for now.  Choose the leading finite branch,
    `Spin(10)`, and supply the missing action-level completion data requested
