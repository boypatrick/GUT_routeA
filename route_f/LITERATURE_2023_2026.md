...
separates oscillation and concentration for an `L^p`-bounded gradient
sequence.  Kružík characterizes gradient-generated boundary concentration
for one declared `p`-growth.  These results do not produce a modification of
one `S3`-valued map preserving the simultaneous endpoint `L2` graph of
`Du,M2,M3` and relative degree.

The graph-tight annulus theorem, exact Dirac variance identity, rank-two
microball scaling, and WZ-capacity/topology alternative are derived directly
in the repository.  No cited source is treated as proving GTA, full `mu=0`,
or regularity of the selected minimizer.

| Source | Relevant result | Transfer boundary |
|---|---|---|
| Fonseca, Müller, and Pedregal, [Analysis of concentration and oscillation effects generated by gradients](https://doi.org/10.1137/S0036141096306534), SIAM J. Math. Anal. 29 (1998) 736 | Separates ordinary oscillation from concentration for gradients bounded in one `L^p` scale and develops the associated Young-measure representation. | It does not preserve the nonlinear simultaneous graph `(Du,M2,M3)`, the sphere target, and relative degree under one endpoint modification. |
| Kružík, [Quasiconvexity at the boundary and concentration effects generated by gradients](https://arxiv.org/abs/1009.0795), ESAIM COCV 19 (2013) 679 | Characterizes DiPerna--Majda measures generated by gradients, including boundary concentration for a fixed `p`-growth compactification. | Boundary concentration theory for `Du` alone is not the GTA trace theorem for separately `L2` complete minors. |

## AP-E16 tangent/Young-measure boundary (superseded by AP-E17)

AP-E16 transfers no external theorem.  Its common component-tangent
construction, multi-order scaling obstruction, rank-one cutoff algebra, and
homogeneous sphere-valued Young-measure counterexample are derived directly
in the repository.

The literature boundary is stricter than at AP-E15.  A theorem giving
Lebesgue-a.e. differentiation or sequence-uniform weighted cutoff decay would
still not remove the explicit diffuse pure-base Young measure.  The missing
result must preserve the boundary trace and relative homotopy while
transferring normalized **full local quasiminimality** from a recovery
sequence to its tangent Young measure.  None of the sources below is cited as
providing that theorem for simultaneous `L2` control of first, second, and
third complete minors, nor as treating the separate nonvolumetric
concentration branch.

## AP-E15 defect-support boundary (superseded by AP-E16)
...
| Particle Data Group, [Review of Particle Physics 2024](https://pdg.lbl.gov/2024/) | Gives the current `alpha_s(MZ)=0.1180+-0.0009` reference and the experimental context for high-scale running. | P2 uses current central inputs and propagates their errors; it does not identify the resulting small covariance with the much larger model-threshold uncertainty. |
| Zhang, [Imperfect Axion Precludes the Domain Wall Problem](https://arxiv.org/abs/2305.15495) (2023) | Develops thermal explicit-PQ-breaking mechanisms that can prevent dangerous post-inflation walls while retaining a strong-CP solution in a constrained window. | Keep as an alternative to the new `P54PQ-F10` anomaly repair.  No bias operator is promoted without a common wall-decay, fifth-force, EDM, and axion-quality calculation. |
