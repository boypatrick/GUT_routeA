...
\item[Deferred audits (executed July 2026; results in repository
ledgers).]
The full flavor fit, the \(d=5\) proton-decay Wilson tensors, the
threshold/proton companion audits, and the source-sector dynamics ran
to completion as standalone machine-checked scripts (ledger pairs under
\texttt{output/audit0..9/}; collected with a branch map and explicit
kill criteria in the companion reconstruction note,
\texttt{route\_E/tex/route\_e\_first\_principles.tex}, Sec.~``Dynamics
Audits and Falsifiability'').  Net outcome: the benchmark
supersymmetric minimal slice of this conditional branch is EXCLUDED by
...
survives and is falsifiable at Hyper-Kamiokande; the theorem core of
this note is untouched.  The non-supersymmetric re-derivation
(vacuum, source selection, contact re-extraction, leptogenesis) was
subsequently executed at the structural level (ledgers
\texttt{output/audit9/dyn9b*.json}); a machine-verified invariance
theorem shows the normalized contact content is branch-independent,
with only the suppression scale \(M_*\) branch-local.
