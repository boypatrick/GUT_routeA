...
Route-D local string-placement/instanton arithmetic.  The Audit 0/1/1b/4a
files are convention, rank-contract, and schema cards for future work, while
Audit 0.5 is only a finite phase-transfer diagnostic.  None of these files is
a publication-grade phenomenology result.  All publication-grade phenomenology
remains outside this manifest until the companion audits are restored.
...
doublet/triplet mass-block schema, including the required \(5\times5\) triplet
inverse-block placeholder.  It still does not solve the F/D-flat vacuum,
import the Aulakh--Girdhar/BMSV mass-matrix entries, export scalar-Hessian
Goldstone eigenvectors, or provide a non-placeholder heavy spectrum.  Thus
Audit 3 remains blocked on a source spectrum, and Audit 2 remains blocked on
both physical flavor rotations and triplet inverse data.
