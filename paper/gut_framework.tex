...
\(SL(2)\)-invariant second-transvectant direction \(K_{\rm tr}\).  The paper
also records a Route-B candidate: a visible-singlet sterile transvectant
messenger can generate the \(\zeta K_{\rm tr}\) contact by a Schur complement
when \(\lambda^2=\zeta\).  The remaining open datum is the hidden phase/radial
sector that supplies this complex coefficient, together with companion audits
for flavor, proton decay, thresholds, and source-sector UV origin.  Detailed
...
\section{\texorpdfstring{Route B: Hidden Transvectant Origin for \(\zeta\)}{Route B: Hidden Transvectant Origin for zeta}}
...
clockwork classes do not predict it.  The conservative Route-B move is
...
\bottomrule
\end{tabular}
\caption{Compact Route-B numerical verification.  The table records only the
algebraic checks used in the lean hidden-\(\zeta\) section; it is not a
replacement for the deferred full flavor/proton/threshold companion audits.}
...
\(K_{\rm tr}\) uniqueness; Route-B Schur complement.\\
conditional inputs &
