...
the same skeleton.  The hidden phase/radial sector remains open.  Companion
audits for flavor, proton decay, thresholds, and source-sector UV origin have
been mechanically executed and ledgered, but they are not a physics closure:
the fail-closed collector records
\texttt{physics\_promotion\_allowed=false}, with the global
non-supersymmetric flavor fit, valid messenger action, recomputed string
pricing, and flavored thermal kinetics still open.
\end{abstract}
...
approximations of this kind are generic for any real phase.
(Retirement note, July 2026: on the refreshed Majorana card produced by
the executed flavor audit, the \(\mathbb Z_{178}\) candidate misses the
refreshed phase by \(4.1\times10^{-3}\ \mathrm{rad}\), \(76\) times the
loose window quoted above; the diagnostic is retained for the frozen
benchmark card only, and the continuous-phase reading stands.  Ledger:
\texttt{output/audit1/dyn4b\_unconditional\_zeta.json}.)  For
...
\begin{observation}[Audit 0.5 visible-spurion phase-transfer no-hit]
\label{obs:audit05-phase-transfer}
Audit 0.5 is a finite phase-transfer diagnostic.  It asks whether the
benchmark contact phase can be inherited from visible spurion invariants in
\(M_V\).  In the local invariant card,
\[
  \begin{aligned}
  I&=0.008692433865+0.018779423508\,i,\\
  J&=-8.062794523\times10^{-5}
     +5.604303361\times10^{-4}\,i .
  \end{aligned}
\]
The finite table
\[
  \arg I,\qquad 2\arg I,\qquad \arg J,\qquad \arg I+\arg J
\]
has no hit in either the loose or tight phase window.  Real single-coefficient
monomials are compared modulo \(\pi\), since the real coefficient may have
either sign; the \(2\arg I\) row is compared modulo \(2\pi\), corresponding to
\(\lambda=cI\) followed by \(\zeta=\lambda^2\) with \(c^2>0\).  The closest
candidate is \(\arg I\), with
\[
  |\Delta\phi|=0.5372532875265\ {\rm rad},
\]
far outside the loose window
\[
  5.424119231661\times10^{-5}\ {\rm rad}.
\]
Thus the simplest zero-parameter and single-real-parameter visible-spurion
phase-transfer mechanisms are excluded for the current benchmark.

This does not exclude a two-real-coefficient fit
\[
  \zeta=c_1 I+c_2J,\qquad c_1,c_2\in\mathbb R,
\]
because the corresponding real \(2\times2\) system is generically solvable.
The raw sizes of \(c_1,c_2\) are convention-dependent because they scale with
the normalization of \(I,J\); the meaningful quantities are the term
magnitudes in the recorded invariant convention.  For the current card,
\[
  c_1=10.37751489216,\qquad c_2=-216.3199491664,
\]
with residual \(1.39\times10^{-17}\).  The fit-cost diagnostics are
\[
  \frac{\max(|c_1I|,|c_2J|)}{|\zeta|}
  =1.646438908603,\qquad
  \frac{|c_1I|+|c_2J|}{|\zeta|}
  =2.585476554589 .
\]
The cancellation index is only \(O(1)\), so this two-coefficient class is not
excluded as a possibility and is not evidence of a contrived cancellation.
It is nevertheless an unconstrained fit rather than a phase prediction.  The
surviving minimal interpretations are a complex hidden coupling, an
axion/radial hidden phase, or an instanton prefactor depending on data beyond
the visible spurion.  Consequently, in Audit 1 the phase \(\arg\zeta\) is
treated as an independent hidden CP parameter unless a later UV mechanism
predicts it before the flavor fit is performed.
\end{observation}
...
\item Optional phase-transfer diagnostic:
\path{code/audit05_phase_transfer_test.py}, with local output
\path{output/audit05/phase_transfer_test.*}.
\item Audit 1 target-table scaffold:
\path{code/audit1_flavor_target_conventions.py}; output
\path{output/audit1/}.
\item Audit 1b rank-contract scaffold:
\path{code/audit1b_covariant_rank_contract.py}; output
\path{output/audit1b/}.
\item Audit 4a source-spectrum scaffold:
\path{code/audit4a_source_spectrum_schema.py}; output
\path{output/audit4a/}.
\item Audit 4a.1 CMSGUT vacuum-branch scaffold:
\path{code/audit4a1_cmsgut_vacuum_branches.py}; output
\path{output/audit4a1/}.
\item Audit 4a.1 CMSGUT mass-export schema:
\path{code/audit4a1_cmsgut_mass_export.py}; output
\path{output/audit4a1/mass\_export\_schema.*}.
\item Audit 4a.1 CMSGUT literature mass import:
\path{code/audit4a1_cmsgut_literature_mass_import.py}; output
\path{output/audit4a1/literature\_mass\_matrices.*}.
...
Route-D local string-placement/instanton arithmetic, together with the
Audit 4a.1 vacuum-cubic, Goldstone-count, mixed chiral/gauge Goldstone smoke,
and numeric triplet-inverse smoke gates.  The Audit 0/1/1b/4a files are convention,
rank-contract, and schema cards for future work, while Audit 0.5 is only a
finite phase-transfer diagnostic.  None of these files is a publication-grade
phenomenology result.  All publication-grade phenomenology remains outside
this manifest until the companion audits are restored.
...
targets as requiring refresh.  Audit 1b fixes only the covariant-subspace
rank contract: it rules out a hollow two-tensor literature pass/fail test with
unknown family basis and requires future projections to use at least three
aligned tensors with tensor-by-tensor residual reporting.  Audit 4a currently
fixes only the source-sector heavy-spectrum schema and records that no
non-placeholder \texttt{heavy\_spectrum.json} has yet been exported.  Audit
4a.1 opens a CMSGUT-like \(210\oplus126\oplus\overline{126}\oplus10\)
stage-1 scaffold in Pati--Salam blocks.  It fixes the Aulakh--Girdhar vacuum
convention
\[
  x=-\lambda\omega/m,\qquad
  \xi=\lambda M/(\eta m),\qquad
  8x^3-15x^2+14x-3=-\xi(1-x)^2,
\]
and cross-checks it against the ABMSV convention after the corresponding
\(\omega\)-sign change \cite{AulakhGirdhar2005,AulakhBajcMelfoSenjanovicVissani2004}.
The validator reproduces the source-named \(SU(5)\), flipped
\(SU(5)\times U(1)\), and \(G_{LR}\) special points; it does not claim a full
Pati--Salam special point.  A follow-up Audit 4a.1 mass-export card verifies
the group-theoretic Goldstone count \(45-(8+3+1)=33\) and exports the
doublet/triplet mass-block schema.  The literature-import card then
transcribes the Aulakh--Girdhar \(4\times4\) doublet matrix and \(5\times5\)
triplet matrix from Appendix I of Ref.~\cite{AulakhGirdhar2005}.  The local
BMSV note is used only as a qualitative cross-reference because it does not
contain a second entry table for these matrices.  The same card transcribes
the mixed chiral/gauge blocks \(G,E,F,J,X\).  On a generic F-flat sample their
upper-left chiral subblocks have null singular-value ratios
\(5.46\times10^{-33}\), \(7.95\times10^{-17}\), \(0\),
\(2.07\times10^{-17}\), and \(1.87\times10^{-17}\), respectively, while the
full Dirac/gaugino blocks are non-singular.  The card also numerically inverts
the triplet block with residual norm \(2.24\times10^{-15}\) and specifies the
hand-audit adjugate/cofactor contract for a symbolic inverse.  A follow-up
symbolic-inverse card expands the Audit-2-required entries
\((S_1^{\,1},S_1^{\,2},S_2^{\,1},S_2^{\,2},S_1^{\,4},S_2^{\,4})\) as explicit
Leibniz cofactor rational functions: \(\det T\) has \(120\) signed product
terms, each required minor has \(24\) terms, and the numerical reconstruction
matches the direct inverse with maximum entry error \(1.01\times10^{-15}\).
These are still smoke gates, not a completed source audit: scalar-Hessian
Goldstone eigenvectors and a non-placeholder heavy spectrum remain pending.
The source-basis symbolic entries have also been threaded into a first Audit 2
Wilson-builder contract,
\[
  C_{5L\,ab,cd}=\sum_{(i,j)}S_i^{\,j}(Y_{QQ}^{i})_{ab}(Y_{QL}^{j})_{cd},
  \qquad
  C_{5R\,ab,cd}=\sum_{(i,j)}S_i^{\,j}(Y_{UE}^{i})_{ab}(Y_{UD}^{j})_{cd},
\]
with a deterministic random-tensor gate giving zero symbolic/direct
contraction error at the exported precision.  This is a source-basis algebra
handoff only.  Thus Audit 3 remains blocked on a source spectrum, and Audit 2
remains blocked on physical flavor rotations, triplet Yukawa maps, dressing,
and channel-width reduction, but no longer on the source-basis symbolic
triplet-inverse or source-basis Wilson-contract entries.
...
\item[Dynamics evidence (mechanically executed July 2026; physics closure
open).]
The repository contains standalone machine ledgers for the flavor,
threshold/proton, source-sector, and non-supersymmetric diagnostic lanes
(under \path{output/audit0..9/}), collected with a branch map in the
companion reconstruction note,
\path{route_E/tex/route_e_first_principles.tex}, Sec.~``Dynamics
Audits and Falsifiability''.  Their fail-closed collector records
\path{physics_promotion_allowed=false}: DYN-5 lacks a valid interacting
messenger, DYN-9b-2 is not a global flavor fit for the non-SUSY branch, and
DYN-7/9b-3 lack tau-resolved thermal kinetics.  Thus the benchmark
supersymmetric minimal slice has a conditional finite-scan exclusion by
unification combined with proton decay (a slice-family exclusion, not a
model-class exclusion); a non-supersymmetric intermediate-scale variant
remains a preliminary testable candidate; the theorem core of
this note is untouched.  The non-supersymmetric re-derivation
(vacuum, source pricing, contact rescaling, unflavored leptogenesis) was
executed only at the structural/order-estimate level (ledgers
\path{output/audit9/dyn9b*.json}); a machine-verified scaling theorem
shows that, within the fixed-kernel family and for a uniform positive-real
rescaling \(y>0\), the normalized contact content is invariant while only
the suppression scale \(M_*\) moves.  For complex \(y\), instead,
\(M_*\mapsto |y|^2M_*\) and
\(\zeta\mapsto (y^2/|y|^2)\zeta\), so the contact fraction is invariant but
the phase of \(\zeta\) is not.  No global branch-local flavor fit follows
from this fixed-shape covariance.
